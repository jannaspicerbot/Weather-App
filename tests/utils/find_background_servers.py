        ...
        try:
            ...
            if result.stdout.strip():
                ...
                result2 = subprocess.run(
                    cmd2, capture_output=True, text=True, shell=True
                )
...
def find_node_processes():
    ...
    try:
        ...
        if result.stdout.strip() and "Id" not in result.stdout:
...
def find_python_processes():
    ...
    try:
        ...
        if result.stdout.strip() and not result.stdout.strip().startswith(
            ...
            python_procs = [
                line for line in lines if line.strip() and not line.startswith("Id")
            ]
...
def main():
    ...
    if python_procs:
        print(
            f"⚠️  Found {len(python_procs)} Python process(es) - may include scheduler"
        )
    ...
    if processes_to_kill:
        ...
        if response == "y":
            ...
            print(
                "   Verify by checking if localhost:5173 still loads in your browser."
            )
        else:
        ...
        if python_procs:
            ...
            print(
                "Check Task Manager for 'python.exe' processes and their command lines."
            )
