...
class DiagnosticResults:
    ...
    def print_summary(self):
        ...
        print(
            f"Success Rate: {success_count}/{total} ({(success_count/total)*100:.1f}%)"
        )
        ...
        if current_test and current_test["status_code"] != 200:
            ...
            for test in self.tests:
                if test["name"] != current_test["name"] and test["status_code"] == 200:
                    ...
                    test["name"] != current_test["name"] and test["status_code"] != 200
                ):
...
def test_with_user_agent(api_key, app_key, results):
    ...
    try:
        ...
        print(f"❌ ERROR: {e}")
        results.add_test(
            "Fix #2: Custom User-Agent", 0, duration_ms, error=str(e), headers=headers
        )
        return False
    ...
    for i in range(3):
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                print(
                    f"    Retry-After header: {response.headers.get('Retry-After', 'Not provided')}"
                )
                break
